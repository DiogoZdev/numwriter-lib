...
function write(value, { currency = null, locale = 'en-US' } = {}) {
    conf.locale = locale;
    let plural = '';
    let singular = '';
    let smallPlural = '';
    ...
    if (value === 0)
        return numbers[conf.locale].units[0];
...
function convertIntegerToWords(number) {
    if (number < 20)
        return numbers[conf.locale].units[number];
    if (number < 100)
        return `${numbers[conf.locale].tens[Math.floor(number / 10)]}${numbers[conf.locale].units[number % 10] ? ` ${words[conf.locale].connection} ` + numbers[conf.locale].units[number % 10] : ""}`;
    if (number === 100)
        return getHundred();
    if (number < 1000) {
